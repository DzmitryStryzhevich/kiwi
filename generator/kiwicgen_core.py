...
def normalize_ports(ports: Iterable[str] | str) -> tuple[str, ...]:
    ...
    if not requested:
        raise CodegenError("At least one target port must be selected.")
...
def _profile_version_constraint() -> str:
    ...
    try:
        ...
        raise CodegenError(f"Invalid kiwicgen version: {__version__!r}.") from exc
...
def _require_sequence(value: Any, field_name: str) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise CodegenError(f"Profile field '{field_name}' must be a list.")
    return value
...
def load_profile(path: str | pathlib.Path) -> GenerationConfig:
    ...
    # Milestone 2: establish compatibility with the running generator before
    # consuming any profile-owned generation options.
    if PROFILE_VERSION_KEY not in raw:
        raise CodegenError(f"Profile field '{PROFILE_VERSION_KEY}' is required.")
...
def _port_root_dir(
    ...
    if config.split_into_port_dir:
        return module_dir / "portable" / _port_slug(port)
    return module_dir


def _port_header_dir(
    module_dir: pathlib.Path,
    config: GenerationConfig,
    port: str,
) -> pathlib.Path:
    """Resolve the backend header destination for the selected layout."""
    root = _port_root_dir(module_dir, config, port)
    return root / "include" if config.split_src_inc_files else root


def _port_source_dir(
    module_dir: pathlib.Path,
    config: GenerationConfig,
    port: str,
) -> pathlib.Path:
...
def _render_port_cmake(
    forms: PrefixForms,
    config: GenerationConfig,
    port: str,
) -> str:
    """Render one backend CMake file for split output layouts."""
    port_slug = _port_slug(port)
    source = (
        f"src/{forms.snake}_osal_{port_slug}.c"
        if config.split_src_inc_files
        else f"{forms.snake}_osal_{port_slug}.c"
    )
    ...
    return "\n".join(
        [
            ...
            "",
            f"add_library({forms.snake}_osal_{port_slug} STATIC",
            f"    {source}",
            ")",
            "",
            f"target_include_directories({forms.snake}_osal_{port_slug}",
            "    PUBLIC",
            f"        {port_include}",
            f"        {base_include}",
            ")",
            "",
            f"target_link_libraries({forms.snake}_osal_{port_slug}",
            "    PUBLIC",
            f"        {forms.snake}_osal",
            ")",
            "",
...
def _format_generated_sources(
    ...
    # Resolve formatter resources once for the complete formatting stage.
...
def generate(
    ...
    *,
    ...
    if unsupported_ports:
        ...
        raise CodegenError(
            f"Port generation is not implemented for: {requested}."
        )
    ...
    # Milestone 2: render the generic source templates once, then one portable
    # implementation for every selected backend.
    ...
    for port in config.ports:
        ...
        port_outputs = [
            (
                templates_dir / "portable" / template_dir_name / f"template_osal_{port_slug}.h",
                _port_header_dir(module_dir, config, port) / f"{forms.snake}_osal_{port_slug}.h",
            ),
            (
                templates_dir / "portable" / template_dir_name / f"template_osal_{port_slug}.c",
                _port_source_dir(module_dir, config, port) / f"{forms.snake}_osal_{port_slug}.c",
            ),
        ]
    ...
    if config.split_into_port_dir and not config.split_src_inc_files:
        ...
        for port in config.ports:
            ...
            port_cmake = _port_root_dir(module_dir, config, port) / "CMakeLists.txt"
        ...
        for port in config.ports:
            ...
            _write_text(
                port_cmake,
                _render_port_cmake(forms, config, port),
                generated,
                log,
            )
    else:
    ...
    # and only when requested by the normalized generation configuration.
