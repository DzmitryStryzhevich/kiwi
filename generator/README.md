...
`kiwicgen-profile-version` is the version of the profile schema. Version 4 adds the generated-code formatting option. Version 3 introduced the `kiwicgen-` naming for newly saved profiles. Legacy versions 1 through 3 remain accepted on load; older underscore-based version keys are accepted as migration aliases. Newly saved profiles always use the version 4 key shown above. The profile schema version is independent of the OSAL implementation version.
...
`--module-prefix` and `--port` can replace values loaded from a profile. Positive API/layout switches can enable options in addition to those already enabled by the profile. The explicit formatting switches can enable or disable generated-code formatting loaded from a profile.
...
Loading a profile updates the module prefix, port, implemented API checkboxes, layout checkboxes and generated-code formatting option. The output folder remains a local runtime choice and is not read from the YAML profile.
...
Generation itself is still synchronous in the current GUI implementation. The dedicated logging listener serializes messages, but moving generation to a worker thread and making Tk fully responsive during long operations remains a separate performance iteration.
